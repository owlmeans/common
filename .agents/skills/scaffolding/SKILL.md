...
npx @owlmeans/create-app@^0.1.18-rc.41 my-app
```
...
only the package-specific skills wait for `npx @owlmeans/agent-skills@^0.1.18-rc.33` after the
install.
...
primitives + layout/nav/screens). Finish with `npx @owlmeans/agent-skills@^0.1.18-rc.33` to add agent guidance.
...
[[login-methods]], [[login-plugins]], [[agent-skills]]) and every package-specific one. After adding any `@owlmeans/*` dependency, re-run `npx @owlmeans/agent-skills@^0.1.18-rc.33`
