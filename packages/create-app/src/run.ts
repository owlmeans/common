  ...
  if (args.skills) {
    ...
    log(args.install
      ...
      : '\nDeploying harness guidance via @owlmeans/agent-skills (general skills only — re-run'
        + '\n`npx @owlmeans/agent-skills@^0.1.18-rc.32` after installing to add the package-specific ones)…')
    try {
      ...
      if (result.code !== 0) {
        process.stderr.write(`  agent-skills exited with code ${result.code} — you can re-run \`npx @owlmeans/agent-skills@^0.1.18-rc.32\` later.\n`)
      }
    } catch (err) {
