...
describe('create-app — full entrypoint scaffolding', () => {
  test('keeps shared declarations immutable and binds handlers locally', () => {
    ...
    expect(common).toContain('export const appEntrypoints = {')
    expect(common).not.toContain('sessionEntrypoints = protocols(')
    expect(api).toContain("import { bind } from '@owlmeans/server-entrypoint'")
    expect(api).toContain('bind(protocols.api.session.list, handlers.list)')
    expect(api).toContain('bind(protocols.api.session.add, handlers.add)')
    expect(api).toContain('bind(protocols.api.session.remove, handlers.remove)')
  ...
  test('uses one immutable browser binding list', () => {
    ...
    expect(web).toContain('export const appEntrypoints = [')
    expect(web).toContain('...bindAll(protocols.api),')
    expect(web).toContain('bindScreen(protocols.web.session, handler(SessionScreen))')
