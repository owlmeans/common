...
`paymentGate` is an immutable protocol tree. `paymentGateEntrypoints` is built from
`elevate(protocols(paymentGate), [handlers])`; the webhook is public because Stripe signature
verification requires the untouched raw body, while resync carries the ED25519 guard. Never put an
application auth guard on the Stripe webhook.
