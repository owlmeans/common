...
import { LandingGatePreference } from './types.js'
...
const AI_DEPS = {
  [SubProject.Backend]: {
    '@owlmeans/llm': '^0.1.18-rc.36',
    '@owlmeans/agent': '^0.1.18-rc.37',
...
type PersonaSkills = Partial<Record<ViablePersona, ViableSkill[]>>

/**
 * Join persona-skill maps, UNIONING a persona two maps both name.
 *
 * An object spread would keep only the last map's list for that persona — `BusinessAnalyst`
 * carries `AgenticChoice` in an AI case and `LandingGate` from the gate map, and a spread would
 * silently drop whichever came first.
 */
const joinPersonaSkills = (...maps: PersonaSkills[]): PersonaSkills => {
  const out: PersonaSkills = {}
  for (const map of maps) {
    for (const [persona, skills] of Object.entries(map)) {
      const key = persona as ViablePersona
      out[key] = [...new Set([...(out[key] ?? []), ...(skills ?? [])])]
    }
  }
...
/**
 * Who has to know the landing-gate pattern in a case that encourages one.
 *
 * The analyst decides whether a story is gate-worthy and plans the gate's copy; the UX designer
 * shapes the full-scale screen that continues what the guest started; the frontend personas draw
 * the gate, write the handoff and read it back. Nobody else touches it.
 */
const LANDING_GATE_PERSONA_SKILLS: PersonaSkills = {
  [ViablePersona.BusinessAnalyst]: [ViableSkill.LandingGate],
  [ViablePersona.UxDesigner]: [ViableSkill.LandingGate],
  [ViablePersona.LayoutArchitect]: [ViableSkill.LandingGate],
  [ViablePersona.ComponentArchitect]: [ViableSkill.LandingGate],
  [ViablePersona.UiViewCoder]: [ViableSkill.LandingGate],
  [ViablePersona.UiNavCoder]: [ViableSkill.LandingGate],
}

/** The experience layer of a case whose product usually has a step a guest can begin. */
const ENCOURAGE_GATE = { landingGate: LandingGatePreference.Encourage }
...
`.trim(),
    patch: {
      ...
      // can begin anything is the specification's to say.
      experience: { landingGate: LandingGatePreference.Allow },
    },
  },
...
`.trim(),
    patch: {
      packages: {
        ...
        skills: AI_SKILLS,
        personaSkills: joinPersonaSkills(AI_PERSONA_SKILLS, LANDING_GATE_PERSONA_SKILLS),
      },
      experience: ENCOURAGE_GATE,
    },
  },
      ...
      packages: {
        ...
        deps: {
          [SubProject.Backend]: {
            ...
            '@owlmeans/agent-skills': '^0.1.18-rc.36',
          },
        },
        libraries: AI_LIBRARIES,
        skills: [...AI_SKILLS, ViableSkill.TargetAgentTools],
        personaSkills: joinPersonaSkills(AI_PERSONA_SKILLS, LANDING_GATE_PERSONA_SKILLS, {
          [ViablePersona.WorkerCoder]: [ViableSkill.TargetAgentTools],
        }),
      },
      experience: ENCOURAGE_GATE,
    },
...
`.trim(),
    patch: {
      ...
      // A game's first value is PLAYING it, and the play screen is its own front door — a
      // pick-and-preview card on a landing page is the wrong shape for it.
      experience: { landingGate: LandingGatePreference.Discourage },
    },
  },
}
